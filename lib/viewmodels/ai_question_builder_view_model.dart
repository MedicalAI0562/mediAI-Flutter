...
class AIQuestionBuilderViewModel extends ChangeNotifier {
  ...
  List<AIQuestionPaper> get papers {
    ...
    return _papers
        .where((p) => p.bookName.toLowerCase().contains(_searchQuery.toLowerCase()))
  ...
  Future<ApiResponse> generateQuestions(AIQuestionPaper paper, { List<String>? chapterNames }) async {
    return await ApiService.generateQuestions(
      examName: paper.examName,
      topicOrBookName: paper.bookName,
      date: '${paper.date.year.toString().padLeft(4, '0')}-${paper.date.month.toString().padLeft(2, '0')}-${paper.date.day.toString().padLeft(2, '0')}',
      ...
      if (res.status && res.data != null) {
        ...
        if (exams.isNotEmpty) {
          _papers
            ...
            final totalQ = int.tryParse((m['total_questions'] ?? '0').toString()) ?? 0;
