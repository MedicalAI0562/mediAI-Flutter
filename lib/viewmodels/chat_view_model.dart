...
class ChatViewModel extends BaseViewModel {
  ...
  // Add a message
  void addMessage(String message, {bool isUser = true, String? aiType, List<Map<String, dynamic>>? webResults, Map<String, dynamic>? webMeta}) {
    logger.i('Adding message: isUser=$isUser, aiType=$aiType, length=${message.length}');
    logger.d('Message content: "${message.substring(0, message.length > 100 ? 100 : message.length)}${message.length > 100 ? '...' : ''}"');
    ...
    if (message.trim().isNotEmpty) {
      ...
      logger.i('Message added successfully. Total messages: ${_messages.length}');
      notifyListeners();
    } else {
  ...
  void _removeTypingIndicator() {
    _messages.removeWhere((message) => message.text == 'typing...' && !message.isUser);
    notifyListeners();
  }
  ...
  Future<void> startNewSession() async {
    ...
    try {
      ...
      if (response.status && response.data != null) {
        ...
        notifyListeners();
        logger.i('New session started successfully: session_id=$_sessionId, medi_ai_mode=$_mediAiMode');
      } else {
  ...
  Future<void> startNewSessionWithBook(String bookName) async {
    ...
    try {
      ...
      if (response.status && response.data != null) {
        ...
        notifyListeners();
        logger.i('New session started with book "$bookName": session_id=$_sessionId, medi_ai_mode=$_mediAiMode');
      } else {
        // Handle specific error cases
        if (response.code == 401) {
          setError('Please login again to start a new session.');
  ...
  Future<bool> _checkServerConnectivity() async {
    try {
      ...
      // Consider 404 as server reachable (endpoint doesn't exist but server responds)
      if (response.code == 404) {
        logger.i('Health check endpoint not found (404), but server is reachable');
        return true;
      }
      ...
      // If health check fails, try a simple base URL check
      try {
        await ApiService.testServerConnectivity();
        return true; // If testServerConnectivity doesn't throw, server is reachable
      } catch (e2) {
        logger.w('Base server connectivity check also failed: $e2');
        return false;
      }
    }
  }
  ...
  Future<void> sendMessage(String message) async {
    ...
    if (!_isValidSession(_sessionId)) {
      logger.w('No valid session ID found when sending message, starting new session first');
    ...
    addMessage('typing...', isUser: false, aiType: _mediAiMode ? 'medi_ai' : 'common_ai');
    ...
    if (!isServerAvailable) {
      ...
      addMessage('Error: Server is currently unavailable. Please try again later.', isUser: false, aiType: _mediAiMode ? 'medi_ai' : 'common_ai');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (token == null || token.isEmpty) {
        ...
        _removeTypingIndicator();
        addMessage('Error: Please login again to use the chat feature.', isUser: false, aiType: _mediAiMode ? 'medi_ai' : 'common_ai');
        return;
      }

      // Prepare request payload based on current AI mode
      final payload = {
        'message': message,
        'use_mistral_only': !_mediAiMode, // If MediAI mode is ON, use_mistral_only should be false
      };

      // Add session ID to ensure messages are saved to the correct session
      if (_sessionId != null && _sessionId!.isNotEmpty) {
        payload['session_id'] = _sessionId!;
        logger.i('Including session_id in chat payload: $_sessionId');
      ...
      if (_selectedBooks.isNotEmpty) {
        payload['selected_books'] = _selectedBooks;
        ...
        try {
          ...
          if (webSearchResponse.isSuccess && webSearchResponse.data != null) {
            ...
            if (data.containsKey('results') && data['results'] is List) {
              webSearchResults = List<Map<String, dynamic>>.from(data['results']);
              logger.i('Web search returned ${webSearchResults.length} results');
              ...
              logger.w('Web search response missing results key or results is not a list');
            }
          } else {
            logger.w('Web search response not successful: isSuccess=${webSearchResponse.isSuccess}');
          }
        } catch (e) {
      ...
      logger.i('About to send chat request. Web search results count: ${webSearchResults.length}');
              ...
              if (lastMessage is Map<String, dynamic> && 
                ...
                if (content is Map<String, dynamic>) {
                  ...
                  if (mediAiContent.contains('No reply from Mistral') && 
                    ...
                    retryCount++;
                    logger.w('Mistral AI not responding, retrying... (attempt $retryCount/$maxRetries)');
      ...
      if (response.isSuccess) {
        ...
        String aiResponse = 'I apologize, but I couldn\'t process your request at the moment.';
          ...
          if (response.data is Map<String, dynamic>) {
            ...
            if (data.containsKey('messages') && data['messages'] is List) {
              ...
              if (messages.isNotEmpty) {
                ...
                for (int i = messages.length - 1; i >= 0; i--) {
                  ...
                  if (message is Map<String, dynamic> && 
                    ...
                    if (content is Map<String, dynamic>) {
                      ...
                      final msgCommonAi = content['common_ai']?.toString() ?? '';
                      
                      // Check if this message has working content
                      if ((msgMediAi.isNotEmpty && !msgMediAi.contains('No reply from Mistral')) ||
                          (msgCommonAi.isNotEmpty && !msgCommonAi.contains('No reply from Mistral'))) {
                        
                        // Use the first working response we find (most recent)
                        if (msgMediAi.isNotEmpty && !msgMediAi.contains('No reply from Mistral')) {
                          mediAiContent = msgMediAi.trim();
                        }
                        if (msgCommonAi.isNotEmpty && !msgCommonAi.contains('No reply from Mistral')) {
                ...
                logger.i('MediAI content: "${mediAiContent.length > 100 ? mediAiContent.substring(0, 100) + '...' : mediAiContent}"');
                logger.i('Common AI content: "$commonAiContent"');
                logger.i('Current AI mode: ${_mediAiMode ? "MediAI" : "General AI"}');
                logger.i('Found working response: $foundWorkingResponse');
                
                // Log web search results status
                logger.i('Web search results count: ${webSearchResults.length}');
                if (webSearchResults.isNotEmpty) {
                  logger.i('First web search result: ${webSearchResults.first}');
                }
                ...
                if (_mediAiMode) {
                  ...
                  if (mediAiContent.isNotEmpty && !mediAiContent.contains('No reply from Mistral')) {
                    String finalMessage = mediAiContent;
                    
                    // Add web search results if available
                    if (webSearchResults.isNotEmpty) {
                      logger.i('Adding web search results to MediAI response. Results count: ${webSearchResults.length}');
                      String webSearchSection = '\n\n---\n### Relevant Resources:\n\n';
                      
                      // Add metadata if available
                      if (webSearchMetadata != null) {
                        webSearchSection += '**Search Query:** ${webSearchMetadata['query']}\n';
                        webSearchSection += '**Total Results Found:** ${webSearchMetadata['total_results']}\n\n';
                      }
                      ...
                      for (var result in webSearchResults) {
                        ...
                        if (result['medical_relevance_score'] != null) {
                          webSearchSection += '  Relevance Score: ${result['medical_relevance_score']}\n';
                        }
                        webSearchSection += '\n';
                      }
                      ...
                      logger.i('No web search results to add to MediAI response');
                    }
                    
                    addMessage(finalMessage, isUser: false, aiType: 'medi_ai', webResults: webSearchResults.isNotEmpty ? webSearchResults : null, webMeta: webSearchMetadata);
                    logger.i('Using MediAI response (MediAI mode)');
                  } else {
                    // MediAI failed, but check if Common AI has content as fallback
                    if (commonAiContent.isNotEmpty && !commonAiContent.contains('No reply from Mistral')) {
                      addMessage('MediAI is temporarily unavailable, but here\'s some general information that might help:\n\n$commonAiContent', isUser: false, aiType: 'common_ai');
                      logger.i('Using Common AI content as fallback for MediAI mode');
                    } else {
                      // Check if it's specifically a Mistral error
                      if (mediAiContent.contains('No reply from Mistral')) {
                        addMessage('MediAI is currently experiencing technical difficulties. The Mistral AI service is temporarily unavailable. Please try again in a few moments or contact support if the issue persists.', isUser: false, aiType: 'medi_ai');
                      } else {
                        addMessage('I apologize, but MediAI couldn\'t process your request at the moment. Please try again.', isUser: false, aiType: 'medi_ai');
                      }
                      logger.w('No valid MediAI response found - Content: "$mediAiContent"');
                    }
                  }
                } else {
                  ...
                  if (commonAiContent.isNotEmpty && !commonAiContent.contains('No reply from Mistral')) {
                    ...
                    addMessage(finalCommonMessage, isUser: false, aiType: 'common_ai', webResults: webSearchResults.isNotEmpty ? webSearchResults : null, webMeta: webSearchMetadata);
                    logger.i('Added General AI response (General AI mode)');
                  } else {
                    // General AI failed
                    if (commonAiContent.contains('No reply from Mistral')) {
                      addMessage('General AI is currently experiencing technical difficulties. The Mistral AI service is temporarily unavailable.', isUser: false, aiType: 'common_ai');
                    } else {
                      addMessage('General AI couldn\'t process your request at the moment.', isUser: false, aiType: 'common_ai');
                    }
                    logger.w('No valid General AI response found - Content: "$commonAiContent"');
                  }
                  ...
                  if (mediAiContent.isNotEmpty && !mediAiContent.contains('No reply from Mistral')) {
                    ...
                    addMessage(finalMediMessage, isUser: false, aiType: 'medi_ai', webResults: webSearchResults.isNotEmpty ? webSearchResults : null, webMeta: webSearchMetadata);
                    logger.i('Added MediAI response (General AI mode)');
                  } else {
                    // MediAI failed
                    if (mediAiContent.contains('No reply from Mistral')) {
                      addMessage('MediAI is currently experiencing technical difficulties. The Mistral AI service is temporarily unavailable.', isUser: false, aiType: 'medi_ai');
                    } else {
                      addMessage('MediAI couldn\'t process your request at the moment.', isUser: false, aiType: 'medi_ai');
                    }
                    logger.w('No valid MediAI response found - Content: "$mediAiContent"');
                  }
                }
              ...
              logger.i('Using fallback format - looking for response/message/detail');
              ...
              if (_mediAiMode) {
                ...
                addMessage(finalMessage, isUser: false, aiType: 'common_ai', webResults: webSearchResults.isNotEmpty ? webSearchResults : null, webMeta: webSearchMetadata);
                addMessage('MediAI response not available in this format.', isUser: false, aiType: 'medi_ai');
              }
            }
        ...
        if (response.message.contains('Token decode error')) {
          ...
          errorMessage = 'Server returned an unexpected response format. Please try again.';
        }
        
        setError(errorMessage);
        _removeTypingIndicator();
        addMessage('Error: $errorMessage', isUser: false, aiType: _mediAiMode ? 'medi_ai' : 'common_ai');
        logger.e('Chat API error: ${response.message} (Code: ${response.code})');
      }
    } catch (e) {
  ...
  Future<void> initializeAiMode() async {
    try {
      ...
      if (token == null || token.isEmpty) {
        logger.w('No authentication token found, skipping AI mode initialization');
        return;
      }
      
      final response = await ApiService.toggleAiMode(true); // Start with MediAI mode
      ...
      if (response.status && response.data != null) {
        ...
        _sessionId = data['session_id'];
        notifyListeners();
        logger.i('AI mode initialized: medi_ai_mode=$_mediAiMode');
      } else if (response.code == 401) {
        logger.w('Authentication failed during AI mode initialization');
        // Don't clear user data here as it might be called before login
      }
    } catch (e) {
          ...
          if (messages.isNotEmpty) {
            ...
            if (lastMessage is Map<String, dynamic> && 
              ...
              if (content is Map<String, dynamic>) {
                ...
                return {
                  'medi_ai_available': !mediAiContent.contains('No reply from Mistral'),
                  'common_ai_available': !commonAiContent.contains('No reply from Mistral'),
                };
              }
            }
          }
  ...
  Future<void> loadSession(String sessionId) async {
    ...
    logger.i('Available sessions: ${_sessions.map((s) => '${s.id} (${s.title})').toList()}');
    ...
    try {
      ...
      try {
        ...
        logger.w('Session not found in current list, refreshing sessions list...');
      ...
      logger.i('Successfully loaded session: ${session.id} (${session.title}), mediAiMode: $_mediAiMode');
  ...
  Future<void> _loadSessionMessages(String sessionId) async {
    try {
      ...
      if (response.status && response.data != null) {
        ...
        // Handle the response format - it might have messages array
        if (data.containsKey('messages') && data['messages'] is List) {
          final messagesData = data['messages'] as List;
          logger.i('Found ${messagesData.length} messages in session data');
          ...
          for (int i = 0; i < messagesData.length; i++) {
            ...
            if (messageData is Map<String, dynamic>) {
              ...
              logger.i('Message $i - Role: $role, Content type: ${content.runtimeType}');
              
              if (role == 'user' && content is String) {
                // User message
                logger.i('Adding user message: $content');
                addMessage(content, isUser: true);
              } else if (role == 'assistant' && content is Map<String, dynamic>) {
                // AI message - extract the appropriate content based on current mode
                logger.i('Processing assistant message. Current mediAiMode: $_mediAiMode');
                ...
                if (_mediAiMode) {
                  ...
                  aiType = 'medi_ai';
                  logger.i('MediAI mode - extracted medi_ai content length: ${messageText.length}');
                } else {
                  messageText = content['common_ai']?.toString() ?? '';
                  aiType = 'common_ai';
                  logger.i('General AI mode - extracted common_ai content length: ${messageText.length}');
                }
                
                // If the selected AI mode has no content or shows Mistral error, try the other one
                if (messageText.isEmpty || messageText.contains('No reply from Mistral')) {
                  if (_mediAiMode && content['common_ai']?.toString().isNotEmpty == true && !content['common_ai'].toString().contains('No reply from Mistral')) {
                    messageText = content['common_ai'].toString();
                    aiType = 'common_ai';
                  } else if (!_mediAiMode && content['medi_ai']?.toString().isNotEmpty == true && !content['medi_ai'].toString().contains('No reply from Mistral')) {
                    messageText = content['medi_ai'].toString();
                    aiType = 'medi_ai';
                  }
                }
                ...
                if (messageText.isEmpty || messageText.contains('No reply from Mistral')) {
                  if (_mediAiMode) {
                    messageText = 'MediAI is currently experiencing technical difficulties. The Mistral AI service is temporarily unavailable.';
                    aiType = 'medi_ai';
                  } else {
                    messageText = 'The AI service is currently experiencing technical difficulties. The Mistral AI service is temporarily unavailable.';
                    aiType = 'common_ai';
                  }
                }
                
                logger.i('Final message text length: ${messageText.length}, aiType: $aiType');
        ...
        logger.w('No messages found for session $sessionId: ${response.message}');
      }
    } catch (e) {
  ...
  Future<void> refreshCurrentSession() async {
    if (_sessionId != null && _isValidSession(_sessionId)) {
      ...
      logger.w('No valid current session to refresh, loading sessions list instead');
      await loadSessions();
    }
  }
      ...
      for (final endpoint in booksEndpoints) {
        try {
          ...
          if (response.status && response.data != null) {
            ...
            logger.i('Loaded ${_books.length} books successfully from endpoint: $endpoint');
            setLoading(false);
            notifyListeners();
          ...
          if (endpoint == booksEndpoints.last) {
            ...
            final errorMessage = 'Failed to load books. All endpoints unavailable. Please check your internet connection and try again.';
            setError(errorMessage);
  ...
  void toggleBookSelection(String bookName) {
    ...
    notifyListeners();
    logger.i('Book selection toggled: $bookName, selected: ${_selectedBooks.length}');
  }
  ...
  Future<void> selectBooks() async {
    ...
    try {
      ...
      if (response.status) {
        ...
        logger.w('Select books endpoint not available (404), using local fallback');
  ...
  void searchBooks(String query) {
    ...
    _filterBooks();
    notifyListeners();
    logger.i('Searching books with query: "$_searchQuery", found ${_filteredBooks.length} results');
  }
