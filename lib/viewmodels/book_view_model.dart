...
class BookViewModel extends BaseViewModel {
  ...
  List<Book> get paginatedBooks {
    ...
    logger.i('Pagination: Page $_currentPage, Start: $startIndex, End: $endIndex, Total: ${_filteredBooks.length}');
    ...
    print('DEBUG: Returning ${result.length} books for page $_currentPage: ${result.map((b) => b.bookName).toList()}');
    ...
    for (final endpoint in booksEndpoints) {
      try {
        ...
        if (response.status && response.data != null) {
          ...
          continue;
        } else if (response.code == -1 && response.message.contains('timeout')) {
  ...
  Future<void> loadBooksByChapterMode({ required bool chapterMode, bool useAuth = false }) async {
    ...
    try {
      logger.i('loadBooksByChapterMode: chapterMode=$chapterMode, cachedChapter=${_cachedChapterBooks != null}, cachedNonChapter=${_cachedNonChapterBooks != null}');
      // Serve from cache when available
      if (_cachedChapterBooks != null && _cachedNonChapterBooks != null) {
        logger.i('Serving books from cache');
        _books = chapterMode ? List.from(_cachedChapterBooks!) : List.from(_cachedNonChapterBooks!);
      ...
      if (categorized.status && categorized.data is Map<String, dynamic>) {
        ...
        final List<dynamic> chapterRaw = (data['chapter_wise_books'] as List<dynamic>?) ?? [];
        final List<dynamic> nonChapterRaw = (data['non_chapter_wise_books'] as List<dynamic>?) ?? [];
        _cachedChapterBooks = chapterRaw.map((e) => Book.fromJson(e as Map<String, dynamic>)).toList();
        _cachedNonChapterBooks = nonChapterRaw.map((e) => Book.fromJson(e as Map<String, dynamic>)).toList();
      ...
      final fallback = await ApiService.getBooks(endpoint: UrlServices.BOOKS, params: const {}, useAuth: useAuth);
    ...
    try {
      ...
      if (response.status && response.data != null) {
        ...
        if (response.data is Map && response.data.containsKey('books')) {
          ...
          _books = booksList.map((bookJson) => 
            Book.fromJson(bookJson as Map<String, dynamic>)
          ).toList();
          
          _filterBooks();
          _currentPage = 1;
          _hasMoreData = _books.length >= _itemsPerPage;
          logger.i('Search completed: Found ${_books.length} books for query: $query');
        } else {
  ...
  void _performLocalSearch(String query) {
    ...
    notifyListeners();
    logger.i('Local search completed: Found ${_filteredBooks.length} books for query: $query');
  }
  ...
  Future<void> searchBooksWithParams({
    ...
    try {
      logger.i('Searching books with query: $query, limit: $limit, threshold: $threshold');
      final response = await ApiService.searchBooks(
        query: query,
        limit: limit,
        threshold: threshold,
    ...
    try {
      ...
      if (response.status && response.data != null) {
        ...
        if (data is List) {
          ...
          chapters = chapterList?.map((chapter) => chapter.toString()).toList() ?? [];
  ...
  void _loadMockBooks() {
    ...
    _currentPage = 1;
    _hasMoreData = _books.length >= _itemsPerPage;
    print('DEBUG: Loaded ${_books.length} mock books: ${_books.map((b) => b.bookName).toList()}');
