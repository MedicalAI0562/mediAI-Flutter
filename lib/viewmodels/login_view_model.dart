...
class LoginViewModel extends BaseViewModel {
  ...
  Future<void> login(String username, String password, {bool rememberMe = false}) async {
    ...
    final loginEndpoints = [
      UrlServices.LOGIN,        // /api/login (primary)
      UrlServices.LOGIN_ALT,    // /login (fallback)
    ...
    while (retryCount <= maxRetries) {
      for (final endpoint in loginEndpoints) {
        try {
          ...
          if (response.isSuccess) {
            ...
            logger.i('User data saved - Username: $responseUsername, Email: $responseEmail, Mobile: $responseMobileNumber');
            setLoading(false);
            ...
            if (response.code == 400 || response.code == 401 || response.code == 403 || response.code == 422) {
              setError(response.message);
              setLoading(false);
              return; // Exit completely
            }
            // For 500 errors with authentication messages, don't retry - show server message
            else if (response.code == 500 && (response.message.toLowerCase().contains('invalid') || 
                     response.message.toLowerCase().contains('password') || 
                     response.message.toLowerCase().contains('credential') ||
                     response.message.toLowerCase().contains('authentication'))) {
              setError(response.message);
              setLoading(false);
              return; // Exit completely
            }
            // For other server errors (500+), check if we should retry
              ...
              retryCount++;
              logger.w('Server error ${response.code} on $endpoint, retrying... (attempt $retryCount/$maxRetries)');
          ...
          if (endpoint == loginEndpoints.last) {
            ...
            if (retryCount < maxRetries) {
              retryCount++;
              logger.w('All endpoints failed, retrying... (attempt $retryCount/$maxRetries)');
              await Future.delayed(Duration(seconds: retryCount * 2)); // Exponential backoff
              break; // Break out of endpoint loop to retry
            } else {
              setError('Login failed: All endpoints unavailable. Please check your internet connection and try again.');
      ...
      if (retryCount >= maxRetries) {
        setError('Login failed: Unable to connect to server. Please check your internet connection and try again.');
        break;
      }
    }
  ...
  Future<bool> checkAutoLogin() async {
    try {
      ...
      if (rememberMe && isLoggedIn && userToken != null && userToken.isNotEmpty) {
        final username = storage.getUserName() ?? '';
        
        if (username.isNotEmpty) {
          // User is already logged in and remember me is enabled
          logger.i('Auto-login successful: User $username is already logged in');
