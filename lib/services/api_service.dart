...
class ApiService {
  ...
  static void init() {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      onError: (error, handler) {
        logger.e('❌ API Error: ${error.requestOptions.method} ${error.requestOptions.uri}');
        logger.e('💥 Error Message: ${error.message}');
        if (error.response != null) {
          logger.e('📥 Error Response: ${error.response?.statusCode} - ${error.response?.data}');
        }
        handler.next(error);
      },
    ));
  ...
  static Future<void> testServerConnectivity() async {
    try {
      ...
      logger.w('No working health check endpoints found, but server base URL is reachable');
      
    } catch (e) {
      logger.e('Server connectivity test failed: $e');
      if (kIsWeb) {
        logger.e('Web platform detected - this might be a CORS issue. The server needs to allow cross-origin requests.');
      }
    }
  }
  ...
  static Future<ApiResponse> saveExamWithQuestions(Map<String, dynamic> payload) async {
    return await post(
    ...
    try {
      ...
      if (response.statusCode == 404) {
        return ApiResponse(
          ...
          code: 404,
          status: false,
          message: 'The search endpoint was not found. Please check the server configuration.',
          data: null,
        );
      }
      ...
      if (response.statusCode != null && response.statusCode! >= 400 && response.statusCode! < 500) {
        return ApiResponse(
          ...
          status: false,
          message: 'Search request failed with status ${response.statusCode}. Please check your request.',
          data: null,
        );
      }
      ...
      if (response.data != null) {
        ...
        if (responseData is String && responseData.contains('<!DOCTYPE html>')) {
          return ApiResponse(
            ...
            code: 404,
            status: false,
            message: 'The search endpoint was not found. The server returned an HTML page instead of JSON.',
            data: null,
          );
        }

        // Check if response is a simple string (like redirects or simple responses)
        if (responseData is String && responseData.length < 100 && !responseData.startsWith('{')) {
          return ApiResponse(
            version: '0',
            validationErrors: [],
            code: response.statusCode ?? 400,
            status: false,
            message: 'The server returned an unexpected response format: "$responseData". This might indicate a redirect or incorrect endpoint.',
            data: null,
          );
        }
        ...
        if (responseData is Map && (responseData.containsKey('code') || responseData.containsKey('statusCode'))) {
          ...
          return ApiResponse(
            ...
            code: response.statusCode ?? 400,
            status: false,
            message: 'Unexpected search response format from server. Please contact support.',
            data: null,
          );
        }
      }
  ...
  static Future<ApiResponse> getBooks({
    ...
    try {
      ...
      if (useAuth) {
        ...
        logger.i('Auth required (booksDio) - Full token from storage: ${token ?? 'null'}');
      ...
      if (response.statusCode == 404) {
        return ApiResponse(
          ...
          code: 404,
          status: false,
          message: 'The requested endpoint was not found. Please check the server configuration.',
          data: null,
        );
      }
      ...
      if (response.statusCode != null && response.statusCode! >= 400 && response.statusCode! < 500) {
        return ApiResponse(
          ...
          code: response.statusCode!,
          status: false,
          message: 'Request failed with status ${response.statusCode}. Please check your request.',
          data: null,
        );
      }
      ...
      if (response.data != null) {
        ...
        if (responseData is String && responseData.contains('<!DOCTYPE html>')) {
          return ApiResponse(
            ...
            code: 404,
            status: false,
            message: 'The requested endpoint was not found. The server returned an HTML page instead of JSON.',
            data: null,
          );
        }
        ...
        if (responseData is Map && (responseData.containsKey('code') || responseData.containsKey('statusCode'))) {
        ...
        } else if (responseData is Map && responseData.containsKey('status') && responseData.containsKey('books')) {
          ...
          return ApiResponse(
            ...
            code: status ? 200 : 400,
            status: status,
            message: status ? 'Books loaded successfully' : 'Failed to load books',
            data: responseData,
          );
        } else if (responseData is Map && (responseData.containsKey('chapter_wise_books') || responseData.containsKey('non_chapter_wise_books'))) {
          ...
          return ApiResponse(
            ...
            code: response.statusCode ?? 400,
            status: false,
            message: 'Unexpected response format from server. Please contact support.',
            data: null,
          );
        }
      }
  ...
  static Future<ApiResponse> getChapterWiseBooks({ bool useAuth = false }) async {
    ...
    for (final endpoint in endpoints) {
      final res = await getBooks(endpoint: endpoint, params: const {}, useAuth: useAuth);
      if (res.status && res.data != null) {
        return res;
      }
      if (res.code == 404 || (res.message.toLowerCase().contains('not found'))) {
        continue; // try next
      } else if (res.code == -1 && res.message.toLowerCase().contains('timeout')) {
  ...
  static Future<ApiResponse> getChaptersOfBook({
    ...
    final endpoints = [
      '${UrlServices.AI_BOOKS}/$encoded/chapters', // ai/books/{name}/chapters (preferred)
      '${UrlServices.AI_BOOK}/$encoded/chapters',  // ai/book/{name}/chapters
      'api/books/$encoded/chapters',               // api/books/{name}/chapters
      'api/book/$encoded/chapters',                // api/book/{name}/chapters
      'api/v1/books/$encoded/chapters',            // api/v1/books/{name}/chapters
      'api/v1/book/$encoded/chapters',             // api/v1/book/{name}/chapters
      'books/$encoded/chapters',                   // books/{name}/chapters
      'book/$encoded/chapters',                    // book/{name}/chapters
    ];
    ...
    for (final endpoint in endpoints) {
      try {
        final url = '${UrlServices.BASE_URL}/$endpoint';
        final response = await booksDio.get(url, options: Options(headers: headers));
  ...
  static Future<ApiResponse> toggleBooksMode({ required bool chapterMode }) async {
    try {
      final storageService = await StorageService.getInstance();
      final token = storageService.getUserToken();
      if (token == null || token.isEmpty) {
  ...
  static Future<ApiResponse> post({
    ...
    if (useAuth) {
      ...
      logger.i('Auth required - Token from storage: ${token != null ? '${token.substring(0, 10)}...' : 'null'}');
    ...
    try {
      ...
      if (response.statusCode != null && response.statusCode! >= 400) {
        ...
        if (response.statusCode! >= 500) {
          ...
          errorMessage = 'Request failed with status ${response.statusCode}. Please check your request.';
        }
      ...
      if (response.data != null) {
        ...
        if (responseData is Map && (responseData.containsKey('code') || responseData.containsKey('statusCode'))) {
        ...
        } else if (response.statusCode == 200 && responseData is Map && endpoint.contains('web-search')) {
          ...
          if (responseData is Map && responseData.containsKey('exam_id') && responseData.containsKey('questions')) {
            return ApiResponse(
              version: '0',
              validationErrors: [],
          ...
          if (responseData is Map && responseData.containsKey('message') && responseData.containsKey('exam')) {
            return ApiResponse(
              version: '0',
              validationErrors: [],
              code: response.statusCode ?? 200,
          ...
          if (statusCode != null && statusCode >= 500) {
            String errorMessage = 'Server is temporarily unavailable. Please try again later.';
            if (statusCode == 502) {
    ...
    try {
      ...
      if (response.data != null) {
        ...
        if (responseData is Map && (responseData.containsKey('code') || responseData.containsKey('statusCode'))) {
          ...
          if (responseData is Map && responseData.containsKey('status') && responseData.containsKey('message') && responseData.containsKey('data')) {
            return ApiResponse(
              version: '0',
              validationErrors: [],
              code: response.statusCode ?? 200,
      ...
      final response = await _dio.post(
        ...
        options: Options(
          headers: {
            ...
            if (useAuth && userToken != null) 'Cookie': 'user_auth_token=$userToken',
          },
        ),
      );
    ...
    if (dioErr.response?.data is Map<String, dynamic>) {
      ...
      switch (dioErr.type) {
        ...
        case DioExceptionType.connectionError:
          if (kIsWeb) {
            errorMessage = 'Connection failed. This might be due to CORS policy restrictions. Please ensure the server allows cross-origin requests from this domain.';
          } else {
  ...
  static Future<ApiResponse> getSessionMessages(String sessionId) async {
    try {
      ...
      final response = await _dio.get(
        '${UrlServices.BASE_URL}/${UrlServices.SESSION_MESSAGES}/$sessionId',
        options: Options(
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer $token',
  ...
  static Future<void> debugAllEndpoints() async {
    ...
    final endpoints = [
      {'name': 'Health Check', 'url': UrlServices.HEALTH_CHECK, 'method': 'GET', 'auth': false},
      {'name': 'Books (no auth)', 'url': UrlServices.BOOKS, 'method': 'GET', 'auth': false},
      {'name': 'Books (with auth)', 'url': UrlServices.BOOKS, 'method': 'GET', 'auth': true},
      {'name': 'Exams (no auth)', 'url': UrlServices.EXAMS, 'method': 'GET', 'auth': false},
      {'name': 'Exams (with auth)', 'url': UrlServices.EXAMS, 'method': 'GET', 'auth': true},
      {'name': 'Login', 'url': UrlServices.LOGIN, 'method': 'POST', 'auth': false},
      {'name': 'Chat', 'url': UrlServices.CHAT, 'method': 'POST', 'auth': true},
      {'name': 'Generate Questions', 'url': UrlServices.GENERATE_QUESTIONS, 'method': 'POST', 'auth': true},
    ];
  ...
  static Future<void> _testEndpoint(String name, String url, String method, bool useAuth) async {
    try {
  ...
  static Future<Map<String, dynamic>?> generateLessonPlan({
    ...
    try {
      logger.i('Generating lesson plan for book: $bookName with chapters: $chapterNames');
