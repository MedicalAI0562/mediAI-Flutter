...
class AIQuestionBuilderScreen extends StatefulWidget {
  ...
  @override
  State<AIQuestionBuilderScreen> createState() => _AIQuestionBuilderScreenState();
}
...
class _AIQuestionBuilderScreenState extends State<AIQuestionBuilderScreen> {
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            columns: const [
              ...
              DataColumn(label: Text('Book Name', style: TextStyle(color: Colors.black))),
              ...
              DataColumn(label: Text('Total Questions', style: TextStyle(color: Colors.black))),
              DataColumn(label: Text('Marks per Question', style: TextStyle(color: Colors.black))),
              DataColumn(label: Text('Total Marks', style: TextStyle(color: Colors.black))),
            ],
            rows: vm.papers.map((p) {
              return DataRow(cells: [
                DataCell(Text(p.id, style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.examName, style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.bookName, style: const TextStyle(color: Colors.black))),
                DataCell(Text(_formatDate(p.date), style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.startTime.format(context), style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.endTime.format(context), style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.totalQuestions.toString(), style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.marksPerQuestion.toStringAsFixed(2), style: const TextStyle(color: Colors.black))),
                DataCell(Text(p.totalMarks.toStringAsFixed(2), style: const TextStyle(color: Colors.black))),
              ]);
            }).toList(),
          ),
        ),
  ...
  Future<void> _openAddEditSheet(BuildContext context, AIQuestionBuilderViewModel vm, {AIQuestionPaper? existing}) async {
    // Books will be loaded via search when user starts typing
    final formKey = GlobalKey<FormState>();
    final examNameController = TextEditingController(text: existing?.examName ?? '');
    final bookNameController = TextEditingController(text: existing?.bookName ?? '');
    DateTime? date = existing?.date;
    TimeOfDay? startTime = existing?.startTime;
    TimeOfDay? endTime = existing?.endTime;
    final totalQuestionsController = TextEditingController(text: existing?.totalQuestions.toString() ?? '');
    final marksPerQuestionController = TextEditingController(text: existing?.marksPerQuestion.toString() ?? '');
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              existing == null ? 'Add Question Paper' : 'Edit Question Paper',
                              ...
                              child: Row(
                                ...
                                children: [
                                  Radio<AiOption>(
                                    ...
                                    groupValue: aiOption,
                                    onChanged: (v) => setSheetState(() => aiOption = v ?? AiOption.common),
                                  ),
                              ...
                              child: Row(
                                ...
                                children: [
                                  Radio<AiOption>(
                                    ...
                                    groupValue: aiOption,
                                    onChanged: (v) => setSheetState(() => aiOption = v ?? AiOption.mediAi),
                                  ),
                                  const Text('MediAi'),
                                ],
                              ),
                      ...
                      _FormField(label: 'Exam Name', child: TextFormField(
                        controller: examNameController,
                        validator: (v) => (v == null || v.trim().isEmpty) ? 'Required' : null,
                      )),
                      _FormField(label: 'Books Name', child: _BookDropdown(
                        initial: bookNameController.text,
                        // chapterOption removed from UI context; keep book list loading generic
                        chapterOption: chapterOption,
                        onChanged: (val) {
                          bookNameController.text = val ?? '';
                          // No chapter dropdown; chapters will be typed manually
                        },
                      )),
                      _FormField(
                        label: 'Chapters (type names, comma-separated)',
                        child: TextFormField(
                          controller: chapterTextController,
                          maxLines: null,
                          decoration: InputDecoration(
                            hintText: 'e.g. Chapter 1, Chapter 2, Introduction',
                            border: OutlineInputBorder(
                      ...
                      _FormField(label: 'Date', child: OutlinedButton(
                        onPressed: () async {
                      ...
                      Row(
                        children: [
                          Expanded(child: _FormField(label: 'Start Time', child: OutlinedButton(
                            onPressed: () async {
                            ...
                            child: Text(startTime == null ? 'Select' : startTime!.format(context)),
                          ))),
                          SizedBox(width: 3.w),
                          Expanded(child: _FormField(label: 'End Time', child: OutlinedButton(
                            onPressed: () async {
                              final picked = await showTimePicker(
                                context: context,
                            ...
                            child: Text(endTime == null ? 'Select' : endTime!.format(context)),
                          ))),
                        ],
                      ),
                      Row(
                        children: [
                          Expanded(child: _FormField(label: 'Total Questions', child: TextFormField(
                          ...
                          SizedBox(width: 3.w),
                          Expanded(child: _FormField(label: 'Marks per Question', child: TextFormField(
                      ...
                      _FormField(label: 'Total Marks', child: Container(
                        ...
                        children: [
                          Expanded(
                            child: OutlinedButton(
                              ...
                              onPressed: (isGenerating || isPreviewing) ? null : () {
                          ...
                          Expanded(
                            child: OutlinedButton(
                              ...
                              onPressed: (isGenerating || isPreviewing) ? null : () async {
                                if (!formKey.currentState!.validate() || date == null || startTime == null || endTime == null) {
                                ...
                                try {
                                  final paper = AIQuestionPaper(
                                    ...
                                    marksPerQuestion: double.parse(marksPerQuestionController.text),
                                    ...
                                    Navigator.of(context).push(
                                      MaterialPageRoute(
                                        builder: (context) => QuestionPreviewScreen(
                                          ...
                                          paper: paper, // Pass the paper object for saving
                                        ),
                                      ),
                                    );
                                  }
                                } finally {
                                  if (mounted) setSheetState(() => isPreviewing = false);
                                }
                              },
                              child: isPreviewing
                                  ? Row(
                            ...
                            final paper = AIQuestionPaper(
                              ...
                              chapterOption: chapterOption,
                                ...
                                try {
                                  ...
                                  final typedChapters = chapterTextController.text
                                      .split(',')
                                      .map((e) => e.trim())
                                  ...
                                  final res = await vm.generateQuestions(
                                    paper,
                                    chapterNames: (chapterOption == ChapterOption.chapter && typedChapters.isNotEmpty)
                                        ? typedChapters
                                        : null,
                                  );
                                  if (context.mounted) {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(res.status ? 'Question generated successfully' : 'Failed: ${res.message}'),
...
class _PaperCard extends StatelessWidget {
  ...
  const _PaperCard({required this.paper, required this.onEdit, required this.onDelete});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(
                    paper.examName,
                    style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.w600, color: AppColors.primaryPurple),
                  ),
                ),
                IconButton(onPressed: onEdit, icon: const Icon(Icons.edit, color: Colors.blueGrey)),
            ...
            _DetailRow(label: 'Date', value: '${paper.date.day}/${paper.date.month}/${paper.date.year}'),
            _DetailRow(label: 'Time', value: '${paper.startTime.format(context)} - ${paper.endTime.format(context)}'),
            Row(
              children: [
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Questions', style: TextStyle(color: Colors.grey.shade700, fontSize: 10.sp)),
                      Text(paper.totalQuestions.toString(), style: TextStyle(fontWeight: FontWeight.w600, color: Colors.black, fontSize: 12.sp)),
                    ],
                  ),
                ),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Marks/Q', style: TextStyle(color: Colors.grey.shade700, fontSize: 10.sp)),
                      Text(paper.marksPerQuestion.toStringAsFixed(2), style: TextStyle(fontWeight: FontWeight.w600, color: Colors.black, fontSize: 12.sp)),
                    ],
                  ),
                ),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Total', style: TextStyle(color: Colors.grey.shade700, fontSize: 10.sp)),
                      Text(paper.totalMarks.toStringAsFixed(2), style: TextStyle(fontWeight: FontWeight.w600, color: Colors.black, fontSize: 12.sp)),
                    ],
                  ),
                ),
              ],
...
class _DetailRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(width: 26.w, child: Text(label, style: TextStyle(color: Colors.grey.shade700))),
          Expanded(child: Text(value, style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.black))),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: TextStyle(fontSize: 10.sp, color: Colors.grey.shade800)),
...
class _BookDropdown extends StatefulWidget {
  ...
  const _BookDropdown({this.initial, required this.onChanged, required this.chapterOption});
      ...
      builder: (context, bookVm, _) {
        ...
        return Column(
          ...
          children: [
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      subtitle: book.title.isNotEmpty && book.title != book.bookName
                          ? Text(
                              book.bookName,
                              style: TextStyle(
            ...
            if (_isSearching && books.isEmpty && !bookVm.isLoading && !bookVm.hasError)
              Container(
                ...
                Expanded(
                  child: Text(
                    widget.selectedChapters.isEmpty
                        ? (widget.chapters.isEmpty ? 'No chapters found' : 'Select chapters')
                        : '${widget.selectedChapters.length} chapter(s) selected',
                    style: TextStyle(
                      color: widget.selectedChapters.isEmpty ? Colors.grey.shade600 : Colors.black,
                      fontSize: 14.sp,
                    ),
                  ),
                ),
          ...
          Row(
            children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: () {
                    widget.onSelectionChanged(List<String>.from(widget.chapters));
                  },
                  style: OutlinedButton.styleFrom(
