...
class _BookListScreenState extends State<BookListScreen> {
  ...
  Future<void> _confirmSelection() async {
    ...
    final booksToRemove = chatViewModel.selectedBooks.where(
      (bookName) => !_selectedBooks.contains(bookName)
    ).toList();
    ...
    if (chatViewModel.errorMessage.isEmpty) {
      _showSnackBar('Book selected successfully! You can now start chatting with context from the selected book.');
  ...
  Future<void> _startNewChatWithBook(String bookName) async {
    ...
    try {
      ...
      if (chatViewModel.errorMessage.isEmpty) {
        _showSnackBar('New chat started with "$bookName"! You can now start chatting with context from this book.');
        // Switch to chat tab using callback
        if (widget.onBooksSelected != null) {
          widget.onBooksSelected!();
        }
      ...
      _showSnackBar('Failed to start new chat with book. Please try again.', isError: true);
        ...
        children: [
                    ...
                    Consumer<ChatViewModel>(
                      builder: (context, chatViewModel, child) {
                        ...
                        return Column(
                          ...
                          children: [
                            Text(
                              _isSelectionMode ? 'Select Book for Chat' : 'Medical Books Library',
                              style: TextStyle(
                                fontSize: 4.w,
                                fontWeight: FontWeight.bold,
                                color: AppColors.white,
              ...
              Consumer<ChatViewModel>(
                builder: (context, chatViewModel, child) {
                  ...
                  return IconButton(
                    ...
                    tooltip: _isSelectionMode ? 'Exit Selection' : 'Select Books',
                  );
                },
              ),
            ],
          ...
          Consumer<ChatViewModel>(
            builder: (context, chatViewModel, child) {
              ...
              return _isSelectionMode ? Column(
                children: [
                  SizedBox(height: 2.h),
                  Row(
                    children: [
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: _selectedBooks.isNotEmpty ? () {
                            print('DEBUG: Clear Selection button pressed');
                            _clearSelection();
                          } : null,
                          icon: Icon(
                            Icons.clear,
                      ...
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: _selectedBooks.isNotEmpty ? () {
                            print('DEBUG: Confirm button pressed with ${_selectedBooks.length} selected books');
                            _confirmSelection();
                          } : null,
                          icon: Icon(
                            Icons.check_circle,
        ...
        return Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: Icon(Icons.search, color: AppColors.primaryPurple, size: 4.w),
              ...
              enabledBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(6.w),
                borderSide: BorderSide(color: AppColors.primaryPurple.withOpacity(0.5)),
              ),
  ...
  Widget _buildSearchStatus(BookViewModel bookViewModel) {
    ...
    print('Search Status Debug: query="${bookViewModel.searchQuery}", loading=${bookViewModel.isLoading}, typing=$_isTyping');
    
    // Show status if there's a search query, loading, or typing
    if (bookViewModel.searchQuery.isEmpty && !bookViewModel.isLoading && !_isTyping) {
      return const SizedBox.shrink();
    }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          ] else if (bookViewModel.isLoading && bookViewModel.searchQuery.isNotEmpty) ...[
            SizedBox(
              width: 3.w,
              height: 3.w,
              child: CircularProgressIndicator(
  ...
  Widget _buildBookItem(Book book) {
    ...
    return GestureDetector(
      onTap: _isSelectionMode ? () {
        print('DEBUG: Book item tapped: ${book.bookName}, selection mode: $_isSelectionMode');
        HapticFeedback.lightImpact();
        _toggleBookSelection(book.bookName);
      } : () {
        print('DEBUG: Book item tapped to start new chat: ${book.bookName}');
        HapticFeedback.lightImpact();
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: isSelected 
                      ? AppColors.primaryPurple 
                      : Colors.transparent,
                  borderRadius: BorderRadius.circular(3.w), // Circular for radio button
                  border: Border.all(
                    color: isSelected 
                        ? AppColors.primaryPurple 
                        : Colors.grey[400]!,
  ...
  Widget _buildPaginationControls(BookViewModel bookViewModel) {
    ...
    print('Pagination Debug: Total pages: $totalPages, Current page: ${bookViewModel.currentPage}');
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: Text(
              'Previous',
              style: TextStyle(
                color: bookViewModel.canGoPrevious() ? AppColors.white : Colors.grey[600],
                fontSize: 2.8.w,
              ),
            ),
          ),
          ...
          ElevatedButton(
            ...
            child: Text(
              'Next',
              style: TextStyle(
                color: bookViewModel.canGoNext() ? AppColors.white : Colors.grey[600],
                fontSize: 2.8.w,
              ),
            ),
          ),
  ...
  void _clearSelectedBook(ChatViewModel chatViewModel) {
    ...
    _showSnackBar('Selected book cleared successfully! The AI mode toggle is now available in the chat screen.');
  }
